...
Player.prototype.ruff = function (cards, sameSide) {
    var game = this.currentTable.game;
...
Player.prototype.followPlay = function (cards, cardList, pointFirst) {
    ...
    var firstHand = game.currentRound.getFirstHand();
    var keepTop = false;
    var playingTrump = cardList[0].isTrump(game.trump, game.rank);
    if(!pointFirst && this.aiLevel >= 2 && firstHand.isTrump && playingTrump) {
    ...
    switch (firstHand.type.cat) {
        ...
        case Hand.COMBINATION.QUADS:
            ...
            if(rnks.length > 0 && rnks[rnks.length-1] > leadingHand.maxRank) {
                var xRank = rnks[rnks.length-1];
                if(this.aiLevel >= 3 && rnks.length > 1
                     && (game.currentRound.allFriendsLeft(game, this) || this.noOpponentCanBeat(game, firstHand.suite))) {
                    rnks.sort(function (a, b) {
                        var aPoint = Card.rankToPoint(a, game.rank);
                        var bPoint = Card.rankToPoint(b, game.rank);
                        if(aPoint === bPoint) return a - b;
                        return bPoint - aPoint;
                    });
                    while((xRank = rnks.shift()) <= leadingHand.maxRank);
                }
                sHand = new SimpleHand(leadingHand.type, xRank, cardList === this.trumps);
                cc = Hand.makeCards(sHand, cardList, game.trump, game.rank);
                cc.forEach(function (c) {
                    cards.push(c);
                });
...
Player.prototype.autoPlayCards = function (isLeading) {
    ...
    if (isLeading) {
        ...
        var sameSide = game.isSameSide(this, leadingPlayer);
        if (cardList.length > firstHand.cardNumber) {
            if (sameSide) {
            ...
            if (firstHand.isTrump) {
                ...
                if(pointFirst) {
                    if(this.aiLevel >= 2 && firstHand.cardNumber > 1 && this.totalCardLeft() <= Config.CARD_NUMBER_ENDPLAY) {
                        ...
                        pointFirst = round.isWinning(game, this);
                    }
                }
                this.duckCards(cards, suite, pointFirst, firstHand.cardNumber);
            } else {
                if(sameSide && this.aiLevel >= 2 && firstHand.cardNumber > 1 && this.totalCardLeft() <= Config.CARD_NUMBER_ENDPLAY) {
                    this.duckCards(cards, suite, true, firstHand.cardNumber);
                    return cards;
                }

                if (sameSide && round.isWinning(game, this)) {
                    if(firstHand.cardNumber < 3 && this.aiLevel >= 2) {
                        if(!round.getLeadingHand().isTrump && this.possibleOpponentRuff(game, suite)) {
                            this.ruff(cards, sameSide);
                            return cards;
                        }
                    }
                    this.duckCards(cards, suite, true, firstHand.cardNumber);
